#include "MultiElemsVertexAttributes.h"
...
namespace jking::scene
{
  ...
  MultiElemsVertexAttributes::MultiElemsVertexAttributes(GLFWwindow *window) : Scene::Scene(window), m_CameraControls(window, m_Camera), m_Rotation(glm::vec3(20.0f, 20.0f, 0.0f)), m_Translation(glm::vec3(0.0f, 0.0f, -3.0f))
  {
    /* Cubes */
    ...
    // enable attributes for this VAO
    GLCall(glEnableVertexAttribArray(0));
    GLCall(glEnableVertexAttribArray(1));

    /* Vertex buffer */
    GLCall(glGenBuffers(1, &m_VBO_Cube_ID));
    ...
    /* Index buffer */
    GLCall(glGenBuffers(1, &m_IBO_Cube_ID));
    GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_IBO_Cube_ID));
    ...
    /* Vertex buffer */
    GLCall(glGenBuffers(1, &m_VBO_Arrow_ID));
    GLCall(glBindBuffer(GL_ARRAY_BUFFER, m_VBO_Arrow_ID));
    GLCall(glBufferData(GL_ARRAY_BUFFER, shapeData.vertexBufferSize(), shapeData.vertices, GL_STATIC_DRAW));

    /* Index buffer */
    GLCall(glGenBuffers(1, &m_IBO_Arrow_ID));
    GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_IBO_Arrow_ID));
    GLCall(glBufferData(GL_ELEMENT_ARRAY_BUFFER, shapeData.indexBufferSize(), shapeData.indices, GL_STATIC_DRAW));
  ...
  void MultiElemsVertexAttributes::OnRender()
  {
    // define data
    int width, height;
    ...
    /* Mount arrow buffers */
    GLCall(glBindBuffer(GL_ARRAY_BUFFER, m_VBO_Arrow_ID));
    // We need to again specify the attrib pointers so they point to the data existing in the m_VBO_Arrow_ID.
    // Otherwise they would still point to the m_VBO_Cube_ID buffer
    GLCall(glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), 0));
    GLCall(glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void *)(sizeof(float) * 3)));
    GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_IBO_Arrow_ID));
  ...
  void MultiElemsVertexAttributes::OnImGuiRender()
  {
  }
}