#include "MultiElemsSingleArrayBuffer.h"
...
namespace jking::scene
{
  ...
  /* This demonstrates how we can use single array buffer to store all data */
  MultiElemsSingleArrayBuffer::MultiElemsSingleArrayBuffer(GLFWwindow *window) : Scene::Scene(window), m_CameraControls(window, m_Camera), m_Rotation(glm::vec3(20.0f, 20.0f, 0.0f)), m_Translation(glm::vec3(0.0f, 0.0f, -3.0f))
  {
    /* Cubes */
    ...
    // We store vertex and index data into single buffer, we have
    // Cube vertex data | Cube index data | Arrow vertex data | Arrow index data
    GLCall(glBufferData(GL_ARRAY_BUFFER,
                        cubeShapeData.vertexBufferSize() + arrowShapeData.vertexBufferSize() + cubeShapeData.indexBufferSize() + arrowShapeData.indexBufferSize(),
                        nullptr,
                        GL_STATIC_DRAW));

    // Cube vertex data
    GLsizeiptr offset = 0;
    GLCall(glBufferSubData(GL_ARRAY_BUFFER, offset,
                           cubeShapeData.vertexBufferSize(),
                           cubeShapeData.vertices));

    // Cube index data
    offset += cubeShapeData.vertexBufferSize();
    GLCall(glBufferSubData(GL_ARRAY_BUFFER,
                           offset,
                           cubeShapeData.indexBufferSize(),
                           cubeShapeData.indices));

    // Arrow vertex data
    offset += cubeShapeData.indexBufferSize();
    GLCall(glBufferSubData(GL_ARRAY_BUFFER,
                           offset,
                           arrowShapeData.vertexBufferSize(),
                           arrowShapeData.vertices));

    // Arrow index data
    offset += arrowShapeData.vertexBufferSize();
    GLCall(glBufferSubData(GL_ARRAY_BUFFER,
                           offset,
                           arrowShapeData.indexBufferSize(),
                           arrowShapeData.indices));
    ...
    GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_VBO_ID));
    ...
    GLCall(glEnableVertexAttribArray(0));
    GLCall(glEnableVertexAttribArray(1));
    GLuint arrowByteOffset = cubeShapeData.vertexBufferSize() + cubeShapeData.indexBufferSize();
    GLCall(glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void *)(arrowByteOffset + sizeof(float) * 0)));
    GLCall(glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void *)(arrowByteOffset + sizeof(float) * 3)));
    GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_VBO_ID));

    /* Set variables */
    m_CubeNumIndices = cubeShapeData.numIndices;
    m_ArrowNumIndices = arrowShapeData.numIndices;
    m_CubeIndexDataByteOffset = cubeShapeData.vertexBufferSize();
    m_ArrowIndexDataByteOffset = arrowByteOffset + arrowShapeData.vertexBufferSize();
  ...
  MultiElemsSingleArrayBuffer::~MultiElemsSingleArrayBuffer()
  {
    glDisable(GL_DEPTH_TEST);
    GLCall(glDeleteBuffers(1, &m_VBO_ID));
  }

  void MultiElemsSingleArrayBuffer::OnUpdate(float deltaTime)
  {
    m_CameraControls.OnUpdate(deltaTime);
  }

  void MultiElemsSingleArrayBuffer::OnRender()
  {
    // define data
    int width, height;
    ...
    GLCall(glDrawElements(GL_TRIANGLES, m_CubeNumIndices, GL_UNSIGNED_SHORT, reinterpret_cast<const void *>(static_cast<std::uintptr_t>(m_CubeIndexDataByteOffset))));
  ...
  void MultiElemsSingleArrayBuffer::OnImGuiRender()
  {
  }
}